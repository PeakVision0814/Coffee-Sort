...
2. 检查线路是否已经连接完成，细节如下： 在M5/Arduino系列机器中，烧录Atom固件时，需要使用USB线将末端的Atom接口与电脑的usb口连接；M5系列机器在烧录nimirobot固件时，则使用USB线将M5stack的侧面接口与电脑的usb口连接即可。
...
- ![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/python_install.png)
...
![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/python_gcode.png)
...
- A2：另外，你可以将机械臂的运动模式更改成插补模式，这样运动轨迹会相对平直。
  ...
  注意一定要开插补之后，在send_coords设置的智能规划路线才有用。 插补是指起始点和终止点之间规划了很多密集的点位，从而达到控制中间段轨迹的效果。

**Q:识别到的目标位置，末端无法到达，怎么判断这个坐标是否可以到达然后处理？**

- A：solve inv kinematics(target coords, current_angles)用这个接口看是否有解就可以了。 solve_inv_kinematics(target_coords, current_angles)
  - 功能 : 将坐标转为角度。
  - 参数：
...
![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/ros_1.png)
...
- A：使用指令拉取：
  ...
  ![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/ros_2.png)

**Q: 运行ROS moveit案例发现报错ImprotError：No module named yaml咋办？**

![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/ros_3.png)
...
- A:使用USB线将M5机械臂与PC连接，打开虚拟机设置→USB设备→添加USB设备→选择串口号QinHeng xxxxx，这个就是机器的串口设备。 如果没有这个设备号，可以通过重新拔插设备获取对应的USB设备号，拔插有串口变化的即对应的机器串口设备号
  ...
  ![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/ros_4.png)

**Q:使用基于mujoco的环境进行仿真训练，因此需要机器人的xml文件**

- A:目前GitHub上只有280JN的xml文件：[280JN](https://github.com/elephantrobotics/mycobot_mujoco)
- 提供给客户如何将dae、urdf类型的文件转换成xml文件的方法给客户，让客户用[meshlab自行转换](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/4-SupportAndService/9.Troubleshooting/[https:/blog.csdn.net/qq_43309940/article/details/128292151)。
...
![软件](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/4-SupportAndService/9.Troubleshooting/9.images/%E8%BD%AF%E4%BB%B6.png)
...
![img](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/resources/4-SupportAndService/9.Troubleshooting/9.images/software_1.png)