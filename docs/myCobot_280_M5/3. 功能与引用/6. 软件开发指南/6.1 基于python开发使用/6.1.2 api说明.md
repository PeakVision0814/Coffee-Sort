...
- 参数:
  - `id` : 输入关节ID（范围1-6）
  - `angle`: 参考[send_angle()](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/3-FunctionsAndApplications/6.developmentGuide/python/2_API.html#32-send_angleid-degree-speed)接口中对应关节的限制信息，不得小于最小值
- 返回值:
  - `1`: 完成
...
- 参数：
  - `id` ：输入关节ID（范围1-6）
  - `angle`：参考[send_angle()](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/3-FunctionsAndApplications/6.developmentGuide/python/2_API.html#32-send_angleid-degree-speed)接口中对应关节的限制信息，不得大于最大值
- 返回值：
  - `1`：完成
...
- **参数**:
  ...
  - ```
    ...
    ：夹爪是否是力控类型，如果没有类型参数，则可以省略此参数。（

    注意：仅当末端 Atom 固件版本 ≥ 6.5 时才支持该参数
...
具体请查看 [TCP/IP](https://docs.elephantrobotics.com/docs/mycobot_280_m5_cn/3-FunctionsAndApplications/6.developmentGuide/python/7_TCPIP.html) 小节。